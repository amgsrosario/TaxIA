...
| Frontend de consulta/admin | Existe backoffice; componente isolado de apresentação de `projectedAnswer` criado (D9), ainda não ligado a rota. | ✅ Componente de apresentação (D9); **falta** integração numa vista activa (futuro). |
| Testes existentes de grounding | Existem (várias suites). | ✅ Cenários críticos cobertos por `DocumentedAnswerCriticalScenariosTest` (D10). |
...
| **D7** | ✅ Projecção por `visibilityLevel` — `AnswerProjectionService` projecta `DocumentedTaxiaAnswer` numa `AnswerProjection` (EXTERNAL/DEMO limpos sem bastidores; INTERNAL com diagnóstico moderado; CURATION_ONLY completo), sem recalcular decisão (regras 24–26); `AnswerProjection` ganhou `visibleAnswerType` por adição; `AdminAIController` devolve `projectedAnswer` (INTERNAL) por adição não quebrante; frontend inalterado; testes verdes. Ver §12.D do [contrato](taxia-documented-answer-contract.md). |
| **D8** | ✅ Resolução da lente de projecção — `VisibilityLevelResolver` centraliza a escolha do `VisibilityLevel`-alvo por endpoint/contexto (`resolveForAdminAsk` → INTERNAL, usado no fluxo; EXTERNAL/DEMO/CURATION_ONLY preparados para integração futura); `AdminAIController` passa a resolver a lente via o serviço em vez do literal; sem autorização nova, sem recalcular decisão, sem frontend; testes verdes. Ver §12.E do [contrato](taxia-documented-answer-contract.md). |
| **D9** | ✅ Frontend da resposta profissional — componente isolado e reutilizável `DocumentedAnswerPanel` (React/TS) apresenta um `AnswerProjection` já projectado pelo backend: cabeçalho com o tipo de resposta, corpo, necessidade de parecer, limitações, avisos e fontes visíveis, com rótulos traduzidos (pt-PT); RESPOSTA_LIMITE/PEDIDO_DE_PARECER tratados como respostas legítimas, não erros. Tipos de vista (`AnswerProjection`, `VisibleSource` e enums) só declaram campos seguros — campos internos nunca são apresentados. "Backend decide, backend projecta, frontend mostra": o frontend não recalcula nem decide. Componente **não** ligado a rota nem à API (respeita o guard rail do cliente); `npm run build` verde. Ver §12.F do [contrato](taxia-documented-answer-contract.md). |
| **D10** | ✅ Testes de cenários críticos — `DocumentedAnswerCriticalScenariosTest` compõe os serviços reais (assessment → decisão → mapper → projecção) e testa a filosofia do produto (C1–C9, D1–D9): fonte forte → consulta documentada limpa; actualidade incerta preserva tipo mas assinala prudência; resposta-limite e pedido de parecer não são erro/não-resposta; OUTDATED/fracas/externas/derivadas não sustentam conclusão limpa; EXTERNAL/DEMO ocultam bastidores; INTERNAL/CURATION_ONLY preservam diagnóstico; projecção nunca recalcula a decisão; `aggregatedRiskLevel` não é inventado; `curationStatus.OUTDATED` ≠ `freshnessStatus.OUTDATED`. Sem infra de testes frontend (validação por build). 87 testes verdes na suite mínima. Ver §12.G do [contrato](taxia-documented-answer-contract.md). |
| **D11** | ✅ Diagnóstico interno — `InternalDiagnostics` (record) + `InternalDiagnosticsBuilder` (`@Service`) descrevem o **caminho técnico** até à conclusão («a resposta externa mostra a conclusão profissional; o diagnóstico interno mostra o caminho técnico até lá»). Só runtime (sem persistência, sem coluna, sem auditoria em BD, sem logs sensíveis); **observa, não decide** (lê `answerType`/`parecerRequirement` da `AnswerDecision`, não recalcula). O campo `DocumentedTaxiaAnswer.internalDiagnostics` passou de `String` para `InternalDiagnostics` (alteração compatível, contida; contrato `AdminAIController` preservado). Sinais: decisão, fontes, risco (ausência de agregado real), actualidade, diversidade, projecção, `hiddenForExternal` (só nomes/tipos) e `warningsInternal` — nunca valores sensíveis. `EXTERNAL`/`DEMO` nunca o expõem; `INTERNAL`/`CURATION_ONLY` preservam. Novo `InternalDiagnosticsBuilderTest`; testes actualizados; **98 testes verdes**. Sem frontend. Ver §12.H do [contrato](taxia-documented-answer-contract.md). |
| **Bloco D** | ✅ **Conceptualmente fechado** com D11. Modelo de resposta profissional documentada materializado (DTO/enums → avaliação de fontes → decisão de prudência → projecção por visibilidade → resolução de lente → apresentação → cenários críticos → diagnóstico interno). Afinação fina futura (scoring/thresholds definitivos, risco agregado real, auditoria persistida) vive dentro deste bloco e **não** reabre C1–C9. |
